...
public class WaveView extends WebView {
    ...
    public void setSpeechAmplitude(float amplitude) {
        loadUrl("javascript:SW9.setAmplitude(\"" + Math.max(amplitude, 0.1f) + "\")");
    }

    public void setSpeechAmplitude(int amplitude) {
        loadUrl("javascript:SW9.setAmplitude(\"" + Math.max(amplitude / 50f, 0.1f) + "\")");
    }
}
